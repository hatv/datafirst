...
[<img src="./media/DataScientistRole.png">](https://github.com/hatv/dataworks/tree/master/datascientist/machinelearning/)
...
2.  **Exploring sales data** (for this lab, the data from IBM sample DB “GOSALES” was used):
    ...
    ![Decision Tree Lab Flow] (./media/Decision-Tree-Lab-Flow.jpg "Decision Tree Lab Flow")

    b.  **Using Association Rules**: **DSX RStudio** 
    
    ![Association Rules Lab Flow] (./media/Association-Rules-Lab-Flow.jpg "Association Rules Lab Flow")
...
1.  Download Lab-DSX-ML.zip archive from the github.com location below and extract the data file (transactions.csv) to your laptop:
    ...
    > <https://github.com/hatv/dataworks/raw/master/3.datascientist/machinelearning/archive/Lab-DSX-ML.zip>
...
1.  In DSX, go to user profile settings ![](./media/User-Profile-in-DSX.png)

2.  Go to manage Bluemix account ![](./media/Manage-bluemix-account.png)

3.  From the Bluemix dashboard catalog menu search for “Object Storage”

4.  Click on the Object Storage Icon ![](./media/ObjectStorage.png)
...
4.  Click on the Apache Spark Service Icon ![](./media/apache-spark-service.png)
...
6.  Ensure that your service is up and running – the page that you are transferred after creating the service shows the status of the service (this is highlighted with the red rectangle) ![](./media/Bluemix-verifying-that-the-service-is-running.jpg)
...
7.  Connect your Apache Spark service to your Object Storage service:
    ...
    b.  Click on Object Storage tab (it is highlighted with the red rectangle): ![](./media/Bluemix-connecting-spark-to-object-storage.jpg)

    c.  Click on “Add Object Storage”: ![](./media/Bluemix-connecting-spark-to-object-storage-adding-storage.jpg)

    d.  In the new screen, switch to Bluemix and select the Object Storage service and the container that you provisioned for this lab: ![](./media/Bluemix-connecting-spark-to-object-storage-confirming.jpg)
...
1. From DSX, select DSX data science context (the button to click on is highlighted with the red rectangle): ![](./media/DSX-domain-button-highlighted.png)

2. Click on the menu icon ![](./media/Left-bar-menu-button.png) on the home page of DSX in Data Science context

3. Select Projects > Default project in the open tool bar: the project management page for "Default project" will be shown
![](./media/Default-project-management.png)
...
> ./labs/ml-lab-installation.ipynb
...
![](./media/Notebook-info-privileges.png)
...
6.  Load the second notebook “Machine Learning with DSX - Lab” (from the file machine-learning-with-DSX-lab.ipynb, or from URL ./labs/machine-learning-with-DSX-lab.ipynb ) by following the same steps 1-5 as above
...
1.  From the loaded notebook “Machine Learning with DSX Lab” click on "Find and add data": ![](./media/Selecting-Data-Sources.jpg)
...
3.  Identify the cell with the implementation of getObjectStorageFileWithCredentials and replace the code to use your provisioned Object Storage service:
    ...
    ![](./media/Replacing-the-object-storage-service-inserting-cell.jpg)

    c.  Clicking on “Insert to code” on Transactions.csv will show the options to insert the code: choose “insert base DataFrame” :

 ![](./media/Inserting-code-into-notebook.jpg)

    d.  Here is a similar code that will be inserted into your new cell: ![](./media/Replacing-the-object-storage-service-the-code-inserted.jpg)
    ...
    e.  Replace the existing implementation of getObjectStorageFileWithCredentials (starts with “&lt;- function” and finishes with the end of block “}”) with the generated code in the new cell for getObjectStorageFileWithCredentials\_&lt;unique sequence&gt;; Here is the example of the highlighted code that needs to be replaced:
     ![](./media/Replacing-the-object-storage-service-highlighted-code.jpg)
     ...
     ![](./media/Replacing-the-object-storage-service-code-replacement.jpg)
...
3.  Execute every code section in the order in which the sections appear by clicking on the button ![](./media/Execute-section.png) or by using the menu Cell&gt; Run Cells
...
3.  Execute every code section in the order in which the sections appear by clicking on the button ![](./media/Execute-section.png) or by using the menu Cell&gt; Run Cells. The lab covers the following actions:
...
![](./media/DSX-domain-button-highlighted.png)
...
1.  Double-click on the name of the file RStudio-apriori-demo-installation.R: RStudio will open the source code:
 > ![](./media/RStudio-Source-code.jpg)

2.  Rename ~/.Rprofile to ~/old.Rprofile: check the file .Rprofile and click "Rename", change the name to old.Rprofile
 > ![](./media/Renaming-rprofile.png)

3.  Quit the current session and start the new one:

    a. Select Session>Quit Session:
    
 > ![](./media/Session-menu.png)
 
    b. Select "Don't Save": 
    
 > ![](./media/Quitting-R-session.png)

    c. After the old session has been finished, start a new session:
    
 > ![](./media/Starting-a-new-R-session.png)
    
4.  Run the code in RStudio-apriori-demo-installation.R using the Run button ![](./media/RStudio-running-source-code.png): please decline the options to update any packages while installing the new packages
...
2.  Execute every code section in the order in which the sections appear by clicking on the button ![](./media/RStudio-running-source-code.png) . The lab covers the following actions:
